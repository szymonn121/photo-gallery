...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: previous } = await supabase.from("photos").select("original_storage_path,thumbnail_storage_path,slug").eq("id", id).maybeSingle();
  ...
  if (parsed.data.is_featured) await supabase.from("photos").update({ is_featured: false }).neq("id", id).eq("is_featured", true);
  const { error } = await supabase.from("photos").update(parsed.data as Database["public"]["Tables"]["photos"]["Update"]).eq("id", id);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { supabase, admin } = await requireAdminApi();
  if (!admin) return NextResponse.json({ error: "Brak autoryzacji." }, { status: 401 });
  const { data: photo } = await supabase.from("photos").select("original_storage_path,thumbnail_storage_path,slug").eq("id", id).maybeSingle();
